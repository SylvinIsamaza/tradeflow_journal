...
const CommentEditor: React.FC<CommentEditorProps> = ({ date, type, initialContent, onSave, onClose }) => {
  ...
  const formats = [
    ...
    'list', 'script', 'indent', 'size', 'color', 'background', 'font', 'align',
    'link', 'image', 'video'
  ];
        ...
        <div className="p-6">
          <div className="mb-6 rounded-xl border border-slate-200 overflow-hidden shadow-sm min-h-[400px] bg-white">
            <ReactQuill
              ...
              modules={modules}
              formats={formats}
              placeholder={type === 'daily' ? 'Write your daily review here...' : 'Write your weekly summary here...'}
              theme="snow"
              style={{ height: '350px' }}
            />
          </div>
